...
class SessionTracker {
    ...
    contextItems = [];
    contextWindowDuration = 120; // 120 seconds
    maxContextItems = 500;
    ...
    detectedCodingQuestion = null;
    codingQuestionSource = null;
    codingQuestionSetAt = null;
    // Rolling buffer for multi-segment interviewer question detection
    recentInterviewerBuffer = [];
    static INTERVIEWER_BUFFER_WINDOW_MS = 5 * 60 * 1000; // 5 minutes
    // Screenshot-detected question stays sticky for 3 min before transcript can override
    static SCREENSHOT_STALE_MS = 3 * 60 * 1000;
    ...
    handleTranscript(segment) {
        ...
        if (segment.speaker === 'user') {
            if ((0, verboseLog_1.isVerboseLogging)() && (Math.random() < 0.05 || segment.final)) {
    ...
    async compactTranscriptIfNeeded() {
        ...
        try {
            ...
            if (this.recapLLM) {
                try {
                    const epochSummary = await this.recapLLM.generate(`Summarize this conversation segment into 3-5 concise bullet points preserving key topics, decisions, and questions:\n\n${summaryInput}`);
