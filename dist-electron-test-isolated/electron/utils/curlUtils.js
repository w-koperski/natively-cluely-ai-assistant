...
exports.imageMimeTypeFromPath = imageMimeTypeFromPath;
exports.injectImageIntoMessages = injectImageIntoMessages;
 ...
 */
function injectImageIntoMessages(body, base64Image, imagePath) {
    if (!base64Image || !Array.isArray(body?.messages))
        return body;
    ...
    if (Array.isArray(lastUser.content)) {
        ...
        const alreadyHasImage = lastUser.content.some((part) => part?.type === "image_url");
        if (alreadyHasImage)
            return body;
 ...
 */
function validateImagePath(imagePath, userDataPath) {
    ...
    const allowedRoots = [
        normalizedUserData,
        node_path_1.default.join(normalizedUserData, 'screenshots').replace(/\\/g, '/'),
        node_path_1.default.join(normalizedUserData, 'extra_screenshots').replace(/\\/g, '/'),
    ].filter(Boolean);
    // Resolve the image path to its real path to detect symlink escapes
    let resolvedPath;
    try {
        resolvedPath = node_fs_1.default.realpathSync(imagePath);
