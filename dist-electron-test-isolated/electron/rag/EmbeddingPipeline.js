...
const EmbeddingProviderResolver_1 = require("./EmbeddingProviderResolver");
...
class EmbeddingPipeline {
     ...
     */
    _isConfigImprovement(prev, next) {
        const hasNew = (prevVal, nextVal) => !prevVal && !!nextVal;
    ...
    async _doInitialize(config) {
        ...
        try {
            const local = new LocalEmbeddingProvider_1.LocalEmbeddingProvider();
        ...
        try {
            this.provider = await EmbeddingProviderResolver_1.EmbeddingProviderResolver.resolve(config);
    ...
    async waitForReady(timeoutMs = 15000) {
        ...
        if (this.initPromise) {
            ...
            await Promise.race([
                this.initPromise,
                new Promise((_, reject) => setTimeout(() => reject(new Error(`Embedding pipeline initialization timed out after ${timeoutMs}ms`)), timeoutMs))
            ]);
            return;
        }
    ...
    async processQueue() {
        ...
        const stuckCount = this.db.prepare(`UPDATE embedding_queue SET status = 'pending' WHERE status = 'processing'`).run().changes;
        if (stuckCount > 0) {
        ...
        try {
            while (true) {
                ...
                const useFallback = pending.retry_count === -1 ||
                ...
                if (!activeProvider) {
                    ...
                    this.db.prepare(`UPDATE embedding_queue SET status = 'pending', error_message = 'No provider available' WHERE id = ?`).run(pending.id);
                ...
                // Mark as processing
                this.db.prepare(`UPDATE embedding_queue SET status = 'processing' WHERE id = ?`).run(pending.id);
                try {
                    if (pending.chunk_id) {
                    ...
                    console.error(`[EmbeddingPipeline] Error processing queue item ${pending.id} ` +
                        `(retry ${newRetryCount}/${MAX_RETRIES}, provider: ${activeProvider.name}):`, error.message);
     ...
     */
    async activateMeetingFallback(meetingId) {
        if (!this.fallbackProvider) {
        ...
        console.warn(`[EmbeddingPipeline] Primary provider exhausted for meeting ${meetingId}. ` +
    ...
    async getEmbeddings(texts) {
        ...
        return new Promise((resolve, reject) => {
            const timer = setTimeout(() => {
                reject(new Error(`[EmbeddingPipeline] embedBatch() timed out after ${EMBED_TIMEOUT_MS}ms for ${texts.length} chunks via ${provider.name}`));
            }, EMBED_TIMEOUT_MS);
            provider.embedBatch(texts).then((results) => { clearTimeout(timer); resolve(results); }, (err) => { clearTimeout(timer); reject(err); });
        });
    }
    /**
    ...
    async getEmbeddingForQuery(text) {
        ...
        return new Promise((resolve, reject) => {
            const timer = setTimeout(() => {
                reject(new Error(`[EmbeddingPipeline] embedQuery() timed out after ${EMBED_TIMEOUT_MS}ms for live-query via ${this.provider.name}`));
            }, EMBED_TIMEOUT_MS);
            this.provider.embedQuery(text).then((result) => { clearTimeout(timer); resolve(result); }, (err) => { clearTimeout(timer); reject(err); });
        });
    }
    /**
     ...
     */
    async embedWithTimeout(provider, text, chunkLabel) {
        return new Promise((resolve, reject) => {
            const timer = setTimeout(() => {
                reject(new Error(`[EmbeddingPipeline] embed() timed out after ${EMBED_TIMEOUT_MS}ms for ${chunkLabel} via ${provider.name}`));
            }, EMBED_TIMEOUT_MS);
            provider.embed(text).then((result) => { clearTimeout(timer); resolve(result); }, (err) => { clearTimeout(timer); reject(err); });
        });
    }
    /**
     * Embed a single chunk using the given provider (defaults to this.provider).
     */
    async embedChunk(chunkId, provider) {
        const p = provider ?? this.provider;
        if (!p)
            throw new Error('No embedding provider');
        // Get chunk text
        const row = this.db.prepare('SELECT cleaned_text, meeting_id FROM chunks WHERE id = ?').get(chunkId);
        if (!row) {
     ...
     */
    async embedMeetingSummary(meetingId, provider) {
        const p = provider ?? this.provider;
        if (!p)
            throw new Error('No embedding provider');
        // Get summary text
        const row = this.db.prepare('SELECT summary_text FROM chunk_summaries WHERE meeting_id = ?').get(meetingId);
        if (!row) {
