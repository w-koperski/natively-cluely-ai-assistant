...
const PhoneMirrorService_1 = require("./services/PhoneMirrorService");
...
function initializeIpcHandlers(appState) {
    ...
    const clearActiveModeOnLicenseLoss = () => {
        try {
            ...
            electron_1.BrowserWindow.getAllWindows().forEach((win) => {
                if (!win.isDestroyed())
                    win.webContents.send('modes-active-cleared');
            });
        ...
        try {
            ...
            if (!imagePaths?.length && typeof message === 'string') {
                ...
                if (identityHit) {
                    ...
                    try {
                        PhoneMirrorService_1.PhoneMirrorService.getInstance().publishUserMessage(String(myStreamId), message);
                    }
                    catch (_) {
                        /* noop */
                    }
                    ...
                    try {
                        PhoneMirrorService_1.PhoneMirrorService.getInstance().publishToken(String(myStreamId), identityHit);
                    }
                    catch (_) {
                        /* noop */
                    }
                    try {
                        PhoneMirrorService_1.PhoneMirrorService.getInstance().publishDone(String(myStreamId), identityHit);
                    }
                    catch (_) {
                        /* noop */
                    }
            ...
            try {
                ...
                for await (const token of stream) {
                    ...
                    try {
                        PhoneMirrorService_1.PhoneMirrorService.getInstance().publishToken(String(myStreamId), token);
                    }
                    catch (_) {
                        /* noop */
                    }
                ...
                if (_chatStreamId === myStreamId) {
                    event.sender.send('gemini-stream-done');
                    try {
                        PhoneMirrorService_1.PhoneMirrorService.getInstance().publishDone(String(myStreamId), fullResponse);
                    }
                    catch (_) {
                        /* noop */
                    }
                ...
                if (_chatStreamId === myStreamId) {
                    ...
                    try {
                        PhoneMirrorService_1.PhoneMirrorService.getInstance().publishError(String(myStreamId), streamError?.message || 'Unknown streaming error');
                    }
                    catch (_) {
                        /* noop */
                    }
    ...
    safeHandle('get-meeting-retention', async () => {
        return SettingsManager_1.SettingsManager.getInstance().get('meetingRetention') ?? 'forever';
    });
    ...
    safeHandle('get-provider-data-scopes', async () => {
        return SettingsManager_1.SettingsManager.getInstance().get('providerDataScopes') ?? {};
    });
    ...
    safeHandle('get-screen-understanding-mode', async () => {
        return SettingsManager_1.SettingsManager.getInstance().getScreenUnderstandingMode();
    });
    ...
    safeHandle('get-technical-interview-vision-first', async () => {
        return SettingsManager_1.SettingsManager.getInstance().getTechnicalInterviewVisionFirst();
    });
    ...
    safeHandle('trial:start', async () => {
        try {
            ...
            if (data.ok && data.trial_token && !data.expired) {
                ...
                const prevSttProvider = cm.getSttProvider();
                cm.setNativelyApiKey(constants_1.TRIAL_SENTINEL_KEY); // sentinel — activates natively model routing
                const newSttProvider = cm.getSttProvider();
                if (newSttProvider !== prevSttProvider) {
                    await appState.reconfigureSttProvider();
                }
                const llmHelper = appState.processingHelper?.getLLMHelper?.();
                if (llmHelper)
                    llmHelper.setNativelyKey(constants_1.TRIAL_SENTINEL_KEY);
            }
    ...
    safeHandle('trial:status', async () => {
        try {
            ...
            if (!token)
                return { ok: false, error: 'no_trial_token' };
    ...
    safeHandle('trial:convert', async (_, choice) => {
        try {
            const { CredentialsManager } = require('./services/CredentialsManager');
            const token = CredentialsManager.getInstance().getTrialToken();
            if (!token)
                return { ok: true }; // no token to report
    ...
    safeHandle('trial:end-byok', async () => {
        try {
            ...
            try {
                const sqliteDb = DatabaseManager_1.DatabaseManager.getInstance().getDb();
                if (sqliteDb) {
                    sqliteDb.exec(`
    ...
    safeHandle('generate-assist', async () => {
        try {
            ...
            if (insight) {
                try {
                    PhoneMirrorService_1.PhoneMirrorService.getInstance().publishAssistantMessage(crypto.randomUUID(), insight, 'Assist');
                }
                catch (_) { }
            }
            return { insight };
    ...
    safeHandle('generate-what-to-say', async (_, question, imagePaths, options) => {
        try {
            ...
            if (answer) {
                try {
                    PhoneMirrorService_1.PhoneMirrorService.getInstance().publishAssistantMessage(crypto.randomUUID(), answer, 'What to Answer');
                }
                catch (_) { }
            }
            return {
    ...
    safeHandle('generate-clarify', async () => {
        try {
            ...
            if (clarification === null) {
                ...
                try {
                    PhoneMirrorService_1.PhoneMirrorService.getInstance().publishAssistantMessage(crypto.randomUUID(), clarification, 'Clarify');
                }
                catch (_) { }
            }
    ...
    safeHandle('generate-code-hint', async (_, imagePaths, problemStatement) => {
        try {
            ...
            if (hint) {
                try {
                    PhoneMirrorService_1.PhoneMirrorService.getInstance().publishAssistantMessage(crypto.randomUUID(), hint, 'Code Hint');
                }
                catch (_) { }
            }
            return { hint };
    ...
    safeHandle('generate-brainstorm', async (_, imagePaths, problemStatement) => {
        try {
            ...
            if (script) {
                try {
                    PhoneMirrorService_1.PhoneMirrorService.getInstance().publishAssistantMessage(crypto.randomUUID(), script, 'Brainstorm');
                }
                catch (_) { }
            }
            return { script };
    ...
    safeHandle('generate-follow-up', async (_, intent, userRequest) => {
        try {
            const intelligenceManager = appState.getIntelligenceManager();
            const refined = await intelligenceManager.runFollowUp(intent, userRequest);
            if (refined) {
                try {
                    PhoneMirrorService_1.PhoneMirrorService.getInstance().publishAssistantMessage(crypto.randomUUID(), refined, 'Follow Up');
                }
                catch (_) { }
            }
    ...
    safeHandle('generate-recap', async () => {
        try {
            ...
            if (summary) {
                try {
                    PhoneMirrorService_1.PhoneMirrorService.getInstance().publishAssistantMessage(crypto.randomUUID(), summary, 'Recap');
                }
                catch (_) { }
            }
            return { summary };
    ...
    safeHandle('generate-follow-up-questions', async () => {
        try {
            ...
            if (questions) {
                try {
                    PhoneMirrorService_1.PhoneMirrorService.getInstance().publishAssistantMessage(crypto.randomUUID(), questions, 'Follow-Up Questions');
                }
                catch (_) { }
            }
            return { questions };
    ...
    safeHandle('submit-manual-question', async (_, question) => {
        try {
            ...
            if (answer) {
                try {
                    PhoneMirrorService_1.PhoneMirrorService.getInstance().publishUserMessage(crypto.randomUUID(), question);
                    PhoneMirrorService_1.PhoneMirrorService.getInstance().publishAssistantMessage(crypto.randomUUID(), answer, 'Answer');
                }
                catch (_) { }
            }
        ...
        try {
            ...
            if (tavilyApiKey) {
                ...
                if (nativelyKey) {
                    ...
                    const trialToken = nativelyKey === constants_1.TRIAL_SENTINEL_KEY ? cm.getTrialToken() : undefined;
    ...
    safeHandle('profile:get-persona', async () => {
        try {
            if (!isProOrTrialActive())
                return { success: false, content: '', error: 'pro_required' };
    ...
    PhoneMirrorService_1.PhoneMirrorService.getInstance().onStatusChange((info) => {
        const win = appState.getMainWindow();
        win?.webContents.send('phone-mirror:status', info);
        try {
            const settingsWin = appState.settingsWindowHelper?.getWindow?.();
    ...
    safeHandle('phone-mirror:get-info', async () => {
        return PhoneMirrorService_1.PhoneMirrorService.getInstance().snapshot();
    });
    safeHandle('phone-mirror:enable', async (_, exposeOnLan) => {
        try {
            return await PhoneMirrorService_1.PhoneMirrorService.getInstance().start({
    ...
    safeHandle('phone-mirror:disable', async () => {
        await PhoneMirrorService_1.PhoneMirrorService.getInstance().stop({ persist: true });
        return { success: true };
    });
    safeHandle('phone-mirror:set-lan', async (_, exposeOnLan) => {
        try {
            return await PhoneMirrorService_1.PhoneMirrorService.getInstance().setExposeOnLan(!!exposeOnLan);
        }
        catch (e) {
    ...
    safeHandle('phone-mirror:rotate-token', async () => {
        try {
            return await PhoneMirrorService_1.PhoneMirrorService.getInstance().rotateToken();
        }
        catch (e) {
    ...
    safeHandle('phone-mirror:push-screenshot', async (_, screenshotPath) => {
        try {
            const imgPath = screenshotPath || (await appState.takeScreenshot(false));
            PhoneMirrorService_1.PhoneMirrorService.getInstance().publishAck('screenshot', 'Screenshot captured — queued for AI');
    ...
    PhoneMirrorService_1.PhoneMirrorService.getInstance().onPhoneCommand(async (cmd) => {
        const win = appState.getMainWindow();
        if (cmd.type === 'action') {
            ...
            const phoneMirror = PhoneMirrorService_1.PhoneMirrorService.getInstance();
            const intelligenceManager = appState.getIntelligenceManager();
            ...
            try {
                const llmHelper = appState.processingHelper.getLLMHelper();
                const stream = llmHelper.streamChat(message, undefined, context, prompts_1.CHAT_MODE_PROMPT);
                let full = '';
                for await (const token of stream) {
                    ...
                    if (!phoneMirror.hasClients() && win?.isDestroyed())
                        break;
                    try {
            ...
            try {
                ...
                PhoneMirrorService_1.PhoneMirrorService.getInstance().publishAck('screenshot', 'Screenshot failed');
            }
        }
    });
}
