...
exports.formatSummaryAsBlock = formatSummaryAsBlock;
/**
 ...
 */
function compressConversation(turns, compressCount = 20) {
    ...
    const toneCounts = { positive: 0, negative: 0, tense: 0, collaborative: 0, informative: 0 };
    for (const t of toCompress) {
...
function summarizeNarrative(turnTexts) {
 ...
 */
function formatSummaryAsBlock(summary) {
    ...
    if (summary.keyDecisions.length > 0) {
        ...
        for (const d of summary.keyDecisions)
            lines.push(`    <decision>${escapeXmlText(d)}</decision>`);
        lines.push('  </decisions>');
    }
    if (summary.importantFacts.length > 0) {
        lines.push('  <key_facts>');
        for (const f of summary.importantFacts)
            lines.push(`    <fact>${escapeXmlText(f)}</fact>`);
        lines.push('  </key_facts>');
    }
    if (summary.actionItems.length > 0) {
        lines.push('  <action_items>');
        for (const a of summary.actionItems)
            lines.push(`    <item>${escapeXmlText(a)}</item>`);
        lines.push('  </action_items>');
    }
    if (summary.questionsAsked.length > 0) {
        lines.push('  <open_questions>');
        for (const q of summary.questionsAsked)
            lines.push(`    <question>${escapeXmlText(q)}</question>`);
        lines.push('  </open_questions>');
    }
...
//# sourceMappingURL=ConversationSummarizer.js.map