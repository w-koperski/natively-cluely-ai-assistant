...
exports.getAnswerShapeGuidance = getAnswerShapeGuidance;
exports.warmupIntentClassifier = warmupIntentClassifier;
        ...
        this.loadingPromise = (async () => {
            try {
                ...
                if (electron_1.app.isPackaged) {
                    ...
                    env.localModelPath = path_1.default.join(process.resourcesPath, 'models');
                }
                else {
                    // Dev mode: allow downloading from HuggingFace Hub
                    env.allowRemoteModels = true;
                    env.cacheDir = path_1.default.join(__dirname, '../../resources/models');
                }
                console.log('[IntentClassifier] Loading zero-shot classifier (mobilebert-uncased-mnli)...');
                this.pipe = await pipeline('zero-shot-classification', 'Xenova/mobilebert-uncased-mnli', { local_files_only: electron_1.app.isPackaged });
 ...
 */
function detectIntentByPattern(lastInterviewerTurn) {
 ...
 */
function detectIntentByContext(recentTranscript, assistantMessageCount) {
 ...
 */
async function classifyIntent(lastInterviewerTurn, recentTranscript, assistantMessageCount) {
 ...
 */
function getAnswerShapeGuidance(intent) {
 ...
 */
function warmupIntentClassifier() {
