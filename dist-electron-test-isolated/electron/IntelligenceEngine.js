...
const DynamicActionEngine_1 = require("./services/dynamic-actions/DynamicActionEngine");
...
class IntelligenceEngine extends events_1.EventEmitter {
    ...
    speculativeTextExpiry = Infinity;
    SPECULATIVE_DEBOUNCE_MS = 350;
    SPECULATIVE_MIN_WORDS = 7;
    SPECULATIVE_MIN_CONFIDENCE = 0.75;
    SPECULATIVE_SIMILARITY_THRESHOLD = 0.75;
    ...
    dynamicActionEngine = null;
    currentSessionId = null;
    currentDynamicActionModeId = null;
    currentDynamicActionTemplateType = null;
    static isNonAnswerSentinel(answer) {
    ...
    maybeSpeculate(segment) {
        ...
        if (confidence < this.SPECULATIVE_MIN_CONFIDENCE ||
    ...
    setDynamicActionContext(params) {
        ...
        if (!this.dynamicActionEngine) {
            this.dynamicActionEngine = new DynamicActionEngine_1.DynamicActionEngine();
        }
    ...
    _setDynamicActionEngineForTest(engine) {
        this.dynamicActionEngine = engine;
    }
    detectAndEmitDynamicActions(segment) {
        if (!this.dynamicActionEngine || !this.currentSessionId
    ...
    async planSuggestionTrigger(trigger) {
        const contextItems = this.session.getContext(180);
        const transcriptContext = contextItems.map(item => item.text).join('\n');
        const preparedTranscript = (0, llm_1.prepareTranscriptForWhatToAnswer)(contextItems.map(item => ({
            role: item.role,
            text: item.text,
        ...
        return (0, llm_1.planNextAssistantAction)({
    ...
    async runPlannerDecision(decision, question) {
     ...
     */
    async runWhatShouldISay(question, confidence = 0.8, imagePaths, options) {
        const now = Date.now();
    ...
    async runCodeHint(imagePaths, problemStatement) {
        ...
        try {
            ...
            const generationId = ++this.currentGenerationId;
            let fullHint = "";
            const stream = this.codeHintLLM.generateStream(imagePaths, questionContext ?? undefined, questionSource, transcriptContext ?? undefined);
            let streamAborted = false;
            for await (const token of stream) {
                if (this.currentGenerationId !== generationId) {
