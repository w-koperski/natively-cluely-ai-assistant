...
const PostCallWorkflow_1 = require("./services/post-call/PostCallWorkflow");
const TelemetryService_1 = require("./services/telemetry/TelemetryService");
...
class MeetingPersistence {
    ...
    async stopMeeting() {
        ...
        try {
            ...
            if (meta && meta.doNotPersist === true)
                doNotPersist = true;
        }
        catch (err) {
    ...
    modeSnapshot) {
        ...
        try {
            TelemetryService_1.telemetryService.track({
        ...
        try {
            ...
            try {
                ...
                if (!targetModeId) {
                    ...
                    const templateType = modeSnapshot?.templateType ?? modesMgr.getModes().find((m) => m.id === targetModeId)?.templateType;
                    const modeName = modeSnapshot?.name ?? modesMgr.getModes().find((m) => m.id === targetModeId)?.name ?? 'Unknown';
                    // Prefer user's customized DB sections; fall back to canonical template
                    const dbSections = modesMgr.getNoteSections(targetModeId);
                    modeNoteSections = dbSections.length > 0
                        ? dbSections
                        : (templateType ? (TEMPLATE_NOTE_SECTIONS[templateType] ?? []) : []);
...
Return ONLY valid JSON (no markdown code blocks):
{
  ...
  "actionItems": ["specific next steps, assigned tasks, or implied follow-ups. If absolutely none found, return empty array"]
                    ...
                    groqSummaryPrompt = llm_1.GROQ_SUMMARY_JSON_PROMPT;
                }
                ...
                if (generatedSummary) {
                    ...
                    try {
                        ...
                        if (modeNoteSections.length > 0 && parsed.sections && typeof parsed.sections === 'object') {
                            ...
                            const sectionsArr = modeNoteSections
                                .map(s => ({
                                title: s.title,
            ...
            summaryData = {
                ...summaryData,
                ...(0, PostCallWorkflow_1.buildPostCallEnhancements)({
        ...
        try {
            ...
            try {
                const enhancements = summaryData || {};
                TelemetryService_1.telemetryService.track({
            ...
            try {
                TelemetryService_1.telemetryService.track({
                    ...
                    modeId: modeSnapshot?.id,
                    durationMs: Date.now() - _postCallStart,
                    properties: { errorClass: error?.constructor?.name ?? 'Unknown' },
                });
            }
            catch { /* non-fatal */ }
        }
