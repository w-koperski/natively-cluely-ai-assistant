...
class DatabaseManager {
        ...
        try {
            ...
            if (!fs_1.default.existsSync(dir)) {
                ...
                try {
                    ...
                    const dbExists = fs_1.default.existsSync(this.dbPath);
                    if (dbExists) {
    ...
    runMigrations() {
        if (!this.db)
            return;
        const version = this.db.pragma('user_version', { simple: true }) || 0;
        ...
        if (version < 2) {
            ...
            for (const sql of columnsToAdd) {
                ...
                catch (e) { /* Column already exists from v1 CREATE */ }
            }
        ...
        if (version < 12) {
            ...
            if (modeExists && !existing) {
                ...
                const insertSection = this.db.prepare('INSERT OR IGNORE INTO mode_note_sections (id, mode_id, title, description, sort_order) VALUES (?, ?, ?, ?, ?)');
        ...
        if (version < 13) {
            ...
            const allModes = this.db.prepare('SELECT id, template_type FROM modes').all();
            const insertSection = this.db.prepare('INSERT OR IGNORE INTO mode_note_sections (id, mode_id, title, description, sort_order) VALUES (?, ?, ?, ?, ?)');
    ...
    getCustomNotes() {
        if (!this.db)
            return '';
        try {
            const row = this.db.prepare('SELECT content FROM profile_custom_notes WHERE id = 1').get();
            return row?.content ?? '';
        }
        catch (e) {
    ...
    saveCustomNotes(content) {
        if (!this.db)
            return;
        try {
            this.db.prepare('INSERT INTO profile_custom_notes (id, content, updated_at) VALUES (1, ?, datetime(\'now\')) ON CONFLICT(id) DO UPDATE SET content = excluded.content, updated_at = excluded.updated_at').run(content);
        }
        catch (e) {
    ...
    getPersona() {
        if (!this.db)
            return '';
        try {
            const row = this.db.prepare('SELECT content FROM profile_persona WHERE id = 1').get();
            return row?.content ?? '';
        }
        catch (e) {
    ...
    savePersona(content) {
        if (!this.db)
            return;
        try {
            this.db.prepare('INSERT INTO profile_persona (id, content, updated_at) VALUES (1, ?, datetime(\'now\')) ON CONFLICT(id) DO UPDATE SET content = excluded.content, updated_at = excluded.updated_at').run(content);
        }
        catch (e) {
    ...
    getNoteSections(modeId) {
        if (!this.db)
            return [];
        try {
            return this.db.prepare('SELECT * FROM mode_note_sections WHERE mode_id = ? ORDER BY sort_order ASC, created_at ASC').all(modeId);
        }
        catch (e) {
    ...
    migrateExistingEmbeddings() {
        ...
        try {
            const chunkRows = this.db.prepare('SELECT id, embedding FROM chunks WHERE embedding IS NOT NULL').all();
            if (chunkRows.length > 0) {
                const insert = this.db.prepare('INSERT OR IGNORE INTO vec_chunks(chunk_id, embedding) VALUES (?, ?)');
        ...
        try {
            const summaryRows = this.db.prepare('SELECT id, embedding FROM chunk_summaries WHERE embedding IS NOT NULL').all();
            if (summaryRows.length > 0) {
                const insert = this.db.prepare('INSERT OR IGNORE INTO vec_summaries(summary_id, embedding) VALUES (?, ?)');
                const migrateAll = this.db.transaction(() => {
     ...
     */
    static KNOWN_DIMS = [768, 1536, 3072];
    ...
    saveMeeting(meeting, startTimeMs, durationMs) {
        if (!this.db) {
        ...
        const runTransaction = this.db.transaction(() => {
            // 1. Insert Meeting
            insertMeeting.run(meeting.id, meeting.title, startTimeMs, durationMs, summaryJson, meeting.date, // Using the ISO string as created_at for sorting simply
            meeting.calendarEventId || null, meeting.source || 'manual', meeting.isProcessed ? 1 : 0);
            // 2. Insert Transcript
            ...
            if (meeting.usage) {
                for (const usage of meeting.usage) {
                    ...
                    insertInteraction.run(meeting.id, usage.type, usage.timestamp, queryText, answerText, metadata);
                }
            }
        });
        try {
    ...
    updateMeetingSummary(id, updates) {
        ...
        try {
            ...
            const row = this.db.prepare('SELECT summary_json FROM meetings WHERE id = ?').get(id);
            if (!row)
                return false;
        ...
        const usage = usageRows.map(row => {
            ...
            if (row.metadata_json) {
                try {
                    ...
                    console.warn('[DatabaseManager] Failed to parse metadata_json for interaction:', row?.id, e);
                }
            }
            return {
                type: row.type,
    ...
    getUnprocessedMeetings() {
        ...
        return rows.map(row => {
            ...
            return {
                ...
                source: row.source,
                isProcessed: false,
                transcript: [], // Fetched separately via getMeetingDetails or manually if needed
                usage: []
            };
        });
    }
