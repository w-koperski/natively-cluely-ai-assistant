...
exports.buildVisionProviders = buildVisionProviders;
exports.isOllamaVisionModel = isOllamaVisionModel;
 ...
 */
function buildVisionProviders(inputs) {
    const credentials = CredentialsManager_1.CredentialsManager.getInstance();
    const providers = [];
...
function geminiFlash(creds, _inputs) {
...
function geminiPro(creds, _inputs) {
    const apiKey = creds.getGeminiApiKey();
    return {
        id: 'gemini_pro',
...
function groqScout(creds, _inputs) {
...
function ollama(creds, _inputs) {
    const baseUrl = creds.getAllCredentials()?.ollamaBaseUrl;
    const ollamaModel = creds.getAllCredentials()?.ollamaModel;
...
function codex(creds, _inputs) {
    const cliPath = creds.getAllCredentials()?.codexCliPath;
    ...
    return {
        ...
        modelId: creds.getAllCredentials()?.codexCliModel,
        isLocal: true,
...
function custom(creds, inputs) {
    ...
    const multimodal = active?.multimodal === true;
...
function readActiveCustomProviderSync() {
    try {
        const g = global;
...
function isLocalOnlyCustomProvider(provider) {
    if (!provider)
        return false;
 ...
 */
async function callLLMHelperVision(providerId, params) {
    const helper = await getActiveLLMHelper();
    if (!helper)
        throw new Error('LLMHelper not initialized');
 ...
 */
async function callOllamaVision(baseUrl, model, params) {
    const { optimized, systemPrompt, userPrompt, signal } = params;
    const data = await promises_1.default.readFile(optimized.path);
 ...
 */
async function getActiveLLMHelper() {
    const g = global;
    if (typeof g.__nativelyGetLLMHelper === 'function') {
        try {
...
//# sourceMappingURL=VisionProviderRegistry.js.map