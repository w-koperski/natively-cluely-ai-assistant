...
exports.GENERAL_VISION_SYSTEM_PROMPT = exports.STRUCTURED_EXTRACTION_SYSTEM_PROMPT = exports.CODE_DEBUG_SYSTEM_PROMPT = exports.TECHNICAL_INTERVIEW_SYSTEM_PROMPT = exports.DIRECT_VISION_SYSTEM_PROMPT = void 0;
exports.isTechnicalModeTemplate = isTechnicalModeTemplate;
exports.buildVisionPrompts = buildVisionPrompts;
exports.DIRECT_VISION_SYSTEM_PROMPT = `You are Natively's screen understanding and live assistance engine.
...
exports.TECHNICAL_INTERVIEW_SYSTEM_PROMPT = `You are Natively's technical interview copilot.
...
exports.CODE_DEBUG_SYSTEM_PROMPT = `You are Natively's code analysis assistant.
...
Never claim details that are not visible in the screenshot.`;
exports.STRUCTURED_EXTRACTION_SYSTEM_PROMPT = `You are Natively's screen understanding engine. Extract structured information from the attached screenshot.
...
exports.GENERAL_VISION_SYSTEM_PROMPT = `You are Natively's screen understanding assistant.
...
function isTechnicalModeTemplate(modeTemplateType) {
    if (!modeTemplateType)
        return false;
...
function buildVisionPrompts(req) {
    ...
    let systemPrompt;
    if (isTechnical) {
        systemPrompt = exports.TECHNICAL_INTERVIEW_SYSTEM_PROMPT;
    }
    else if (req.userAction === 'code_hint') {
        systemPrompt = exports.CODE_DEBUG_SYSTEM_PROMPT;
    }
    else if (wantsDirectAnswer) {
        systemPrompt = exports.DIRECT_VISION_SYSTEM_PROMPT;
    }
    else {
        systemPrompt = exports.STRUCTURED_EXTRACTION_SYSTEM_PROMPT;
    }
    ...
    if (req.activeApp || req.windowTitle) {
        const meta = [];
        if (req.activeApp)
            meta.push(`app=${req.activeApp}`);
        if (req.windowTitle)
            meta.push(`window="${req.windowTitle}"`);
...
//# sourceMappingURL=visionPrompts.js.map