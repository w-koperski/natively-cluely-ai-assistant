...
exports.getImageOptimizer = getImageOptimizer;
...
function applyProviderTweaks(provider, base) {
    switch (provider) {
        case 'ollama':
...
class ImageOptimizer {
    ...
    constructor(tempDirOverride) {
        this.tempDir = tempDirOverride || node_path_1.default.join(node_os_1.default.tmpdir(), 'natively-vision-optimized');
    }
    async ensureTempDir() {
        try {
    ...
    async optimize(sourcePath, opts = {}) {
        ...
        let originalStats;
        try {
            originalStats = await promises_1.default.stat(sourcePath);
        }
        catch (err) {
            throw new Error(`ImageOptimizer: cannot stat source image: ${err?.message || err}`);
        }
        const pipeline = (0, sharp_1.default)(sourcePath, { failOnError: false });
        ...
        const outPath = node_path_1.default.join(this.tempDir, `${(0, uuid_1.v4)()}.${ext}`);
     ...
     */
    async getBuffer(optimized) {
        if (optimized.buffer)
            return optimized.buffer;
        return await promises_1.default.readFile(optimized.path);
    }
    /**
     * Read the optimized image as base64 (no `data:` prefix).
     */
    async getBase64(optimized) {
     ...
     */
    async cleanup(optimized) {
        try {
            await promises_1.default.unlink(optimized.path);
        }
        catch {
            // best-effort
        }
    ...
    async cleanupAll() {
        ...
        for (const [, p] of this.ownedFiles.entries()) {
            tasks.push(promises_1.default.unlink(p).catch(() => undefined));
        }
