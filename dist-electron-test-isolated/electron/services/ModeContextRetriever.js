...
class ModeContextRetriever {
    retrieve(mode, files, options) {
        ...
        for (const candidate of candidates) {
            ...
            if (tokenTotal + tokens > tokenBudget && selected.length > 0)
                continue;
     ...
     */
    async retrieveHybrid(mode, files, options) {
        // Lazily create hybrid retriever on first use
        if (!this._hybridRetriever) {
            const db = DatabaseManager_1.DatabaseManager.getInstance().getDb();
            const dbPath = DatabaseManager_1.DatabaseManager.getInstance().getDbPath();
            if (!db) {
                ...
                ModeHybridRetriever_1.ModeHybridRetriever.emitFallbackTelemetryStatic({
            ...
            const vectorStore = new VectorStore_1.VectorStore(db, dbPath, '');
            const embeddingPipeline = new EmbeddingPipeline_1.EmbeddingPipeline(db, vectorStore);
            this._hybridRetriever = new ModeHybridRetriever_1.ModeHybridRetriever(db, vectorStore, embeddingPipeline);
        }
        const queryText = `${options.query}\n${options.transcript ?? ''}`.trim();
        const hasTranscript = !!options.transcript && options.transcript.trim().length > 0;
    ...
    _hybridRetriever = null;
}
exports.ModeContextRetriever = ModeContextRetriever;
//# sourceMappingURL=ModeContextRetriever.js.map