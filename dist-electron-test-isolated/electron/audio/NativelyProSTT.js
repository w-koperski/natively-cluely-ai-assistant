 ...
 */
class NativelyProSTT extends events_1.EventEmitter {
    apiKey;
    channel; // 'system' | 'mic' — disambiguates concurrent streams per key
    ws = null;
    isActive = false;
    isConnected = false;
    isConnecting = false;
    intentionalClose = false; // set true before deliberate closeUpstream() to suppress auto-reconnect
    sampleRate = 16000;
    audioChannels = 1;
    buffer = [];
    ...
    BUFFER_MAX_CHUNKS = 500;
    bufferOverflowReported = false;
    bufferDroppedChunks = 0;
    ...
    DNS_RETRY_MS = 10_000; // fixed delay for ENOTFOUND — don't burn backoff on DNS blips
    isDnsFailure = false; // true when last error was a DNS resolution failure
    reconnectTimer = null;
    // Cleared only after 5 s of stable connection so backoff actually increases on rapid 1006 loops
    stabilityTimer = null;
    BACKEND_URL = 'wss://api.natively.software/v1/transcribe';
    ...
    static nextSlotByKey = new Map();
    static SLOT_INTERVAL_MS = 3000;
    ...
    setRecognitionLanguage(key) {
        ...
        if (key === 'auto') {
            ...
            this.languageAlternates = 'alternates' in config
                ? config.alternates
                : [];
            console.log(`[NativelyProSTT] Language set: ${key} → ${this.languageBcp47}`, this.languageAlternates.length ? `(alts: ${this.languageAlternates.join(', ')})` : '');
        }
    ...
    connect(skipStagger = false) {
        ...
        const ws = new ws_1.default(this.BACKEND_URL, (0, dnsHelpers_1.streamingStttWsOptions)());
        this.ws = ws;
...
exports.NativelyProSTT = NativelyProSTT;
//# sourceMappingURL=NativelyProSTT.js.map