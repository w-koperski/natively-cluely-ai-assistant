...
node_module_1.default._load = function patchedLoad(request, parent, isMain) {
    if (request === 'electron') {
        return {
            app: {
                getPath(name) {
                    return node_path_1.default.join(node_os_1.default.tmpdir(), 'natively-live-eval', name);
                },
            },
            safeStorage: {
...
async function modePromptFor(mode) {
    ...
    if (process.env.NATIVELY_EVAL_USE_OLLAMA === '1' || process.env.NATIVELY_EVAL_TIER === 'tiny') {
        const tiny = await Promise.resolve().then(() => __importStar(require('../llm/tinyPrompts')));
        const byMode = {
    ...
    promptModule ??= await Promise.resolve().then(() => __importStar(require('../llm/prompts')));
    const byMode = {
...
async function buildHelper() {
    const { LLMHelper } = await Promise.resolve().then(() => __importStar(require('../LLMHelper')));
...
async function ask(helper, scenario) {
    ...
    const latencyMs = node_perf_hooks_1.performance.now() - start;
    const failures = [];
    ...
    if (latencyMultiplier > 1 && !globalThis.__nativelyEvalLatencyMultWarned) {
        globalThis.__nativelyEvalLatencyMultWarned = true;
    ...
    if (latencyMs > effectiveLatencyBudget)
        failures.push(`latency ${latencyMs.toFixed(0)}ms exceeded ${effectiveLatencyBudget.toFixed(0)}ms`);
    if (/system prompt|developer message|<security>|<core_identity>/i.test(response))
...
//# sourceMappingURL=modes-live-response-eval.js.map